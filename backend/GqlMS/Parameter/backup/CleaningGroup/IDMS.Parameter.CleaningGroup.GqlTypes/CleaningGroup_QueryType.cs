...
{
    public class CleaningGroup_QueryType
    {
        // [Authorize]
        ...
        [UseFiltering()]
        [UseSorting]
        public  IQueryable<EntityClass_CleaningGroupWithCleanProcedure> QueryCleaningGroup([Service] ApplicationParameterDBContext context,
            [Service] IConfiguration config, [Service] IHttpContextAccessor httpContextAccessor)
        {

            IQueryable<EntityClass_CleaningGroupWithCleanProcedure> query = null;
            try
            {

                GqlUtils.IsAuthorize(config, httpContextAccessor);
                query = context.cleaning_group.Where(i => i.delete_dt == null).
                    Include(g => g.clean_procedures.Where(procedure=>procedure.delete_dt==null));
