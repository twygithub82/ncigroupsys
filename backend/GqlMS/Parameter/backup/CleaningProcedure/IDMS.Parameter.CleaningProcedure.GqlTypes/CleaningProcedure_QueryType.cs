...
{
    public class CleaningProcedure_QueryType
    {
        // [Authorize]
        ...
        [UseFiltering()]
        [UseSorting]
        public  IQueryable<EntityClass_CleaningProcedureWithSteps> QueryCleaningProcedures([Service] ApplicationParameterDBContext context,
            [Service] IConfiguration config, [Service] IHttpContextAccessor httpContextAccessor)
        {

            IQueryable<EntityClass_CleaningProcedureWithSteps> query = null;
            try
            {

                GqlUtils.IsAuthorize(config, httpContextAccessor);
                query = context.cleaning_procedure.Where(i => i.delete_dt == null).
                    Include(g => g.clean_group).
                    Include(s => s.clean_steps.Where(step=>step.delete_dt==null));
