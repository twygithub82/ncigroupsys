...
export class LocationStatusSummaryPdfComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    this.yards.forEach(y => {
      headers[0].push(y.description);
      const lastColumnIndex = Object.keys(comStyles).length;
      comStyles[lastColumnIndex] = { halign: 'center' };
    });
    headers[0].push(this.translatedLangText.TOTAL);
      ...
      didDrawPage: (data: any) => {
        ...
        if (!pg) {
          ...
          if (pageCount > 1) {
            ...
            PDFUtility.addReportSubTitle_Portrait(pdf, repGeneratedDate, pageWidth, leftMargin, rightMargin,subtitlePos);
            // Utility.addReportTitle(pdf, reportTitle, pageWidth, leftMargin, rightMargin, topMargin+45);
            //  Utility.AddTextAtRightCornerPage(pdf, repGeneratedDate, pageWidth, leftMargin, rightMargin +(bufferTabletWidth/2), 48 , 9);
