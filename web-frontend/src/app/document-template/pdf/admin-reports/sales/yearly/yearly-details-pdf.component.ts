...
import { ManagementReportYearlyRevenueItem, MonthlyProcessData, ManagementReportYearlyInventory } from 'app/data-sources/reports-management';
import { AdminReportYearlySalesReport, InventoryAnalyzer } from 'app/data-sources/reports';
...
export class YearlySalesReportDetailsPdfComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  async exportToPDF_r2(fileName: string = 'document.pdf') {
    ...
    const headers = [[
      ...
      ...(showGateSurcharge ? [{ content: this.translatedLangText.GATEIO, colSpan: 2, styles: { halign: 'center' } }] : []),
      ...(showPreinspectSurcharge ? [{ content: this.translatedLangText.PREINSPECTION, colSpan: 2, styles: { halign: 'center' } }] : []),
      ...(showLoloSurcharge ? [{ content: this.translatedLangText.LOLO, colSpan: 2, styles: { halign: 'center' } }] : []),
      ...(showStorageSurcharge ? [{ content: this.translatedLangText.STORAGE, colSpan: 2, styles: { halign: 'center' } }] : []),
      ...(showSteamSurcharge ? [{ content: this.translatedLangText.STEAM, colSpan: 2, styles: { halign: 'center' } }] : []),
      ...(showCleanSurcharge ? [{ content: this.translatedLangText.CLEANING, colSpan: 2, styles: { halign: 'center' } }] : []),
      ...(showResidueSurcharge ? [{ content: this.translatedLangText.RESIDUE, colSpan: 2, styles: { halign: 'center' } }] : []),
      ...(showRepairSurcharge ? [{ content: this.translatedLangText.REPAIR, colSpan: 2, styles: { halign: 'center' } }] : []),
    ],
    [
    ...
    // =========================
    const comStyles: any = {
      0: { halign: 'center', minCellHeight: minHeightBodyCell },
    ...
    const addPairCols = () => {
      comStyles[colIdx++] = { halign: 'center', minCellHeight: minHeightBodyCell };
      comStyles[colIdx++] = { halign: 'center', minCellHeight: minHeightBodyCell };
    };
    ...
    startY = await PDFUtility.addHeaderWithCompanyLogoWithTitleSubTitle_Portrait(
      pdf,
      pageWidth,
      topMargin,
      bottomMargin,
    ...
    for (const monthData of grpData.monthlyData) {
      ...
      let rowTotal =
        (monthData.in_out?.cost || 0) +
        (monthData.lolo?.cost || 0) +
        (monthData.storage?.cost || 0) +
        (monthData.steaming?.cost || 0) +
        (monthData.cleaning?.cost || 0) +
        (monthData.residue?.cost || 0) +
        (monthData.repair?.cost || 0);

      total_all_cost += rowTotal;
      avgCounter++;
      ...
      data.push([
        ...
        ...(showGateSurcharge ? [monthData.in_out?.count || '', Utility.formatNumberDisplay((monthData.in_out?.cost || 0))] : []),
        ...(showPreinspectSurcharge ? [monthData.preinspection?.count || '', Utility.formatNumberDisplay((monthData.preinspection?.cost || 0))] : []),
        ...(showLoloSurcharge ? [monthData.lolo?.count || '', Utility.formatNumberDisplay((monthData.lolo?.cost || 0))] : []),
        ...(showStorageSurcharge ? [monthData.storage?.count || '', Utility.formatNumberDisplay((monthData.storage?.cost || 0))] : []),
        ...(showSteamSurcharge ? [monthData.steaming?.count || '', Utility.formatNumberDisplay((monthData.steaming?.cost || 0))] : []),
        ...(showCleanSurcharge ? [monthData.cleaning?.count || '', Utility.formatNumberDisplay((monthData.cleaning?.cost || 0))] : []),
        ...(showResidueSurcharge ? [monthData.residue?.count || '', Utility.formatNumberDisplay((monthData.residue?.cost || 0))] : []),
        ...(showRepairSurcharge ? [monthData.repair?.count || '', Utility.formatNumberDisplay((monthData.repair?.cost || 0))] : []),
    ...
    data.push([
     this.translatedLangText.TOTAL,
      ...
      ...(showGateSurcharge ? [(this.repData?.gate_yearly_sales?.total_count || 0), Utility.formatNumberDisplay((this.repData?.gate_yearly_sales?.total_cost || 0))] : []),
      ...(showPreinspectSurcharge ? [(this.repData?.preinspection_yearly_sales?.total_count || 0), Utility.formatNumberDisplay((this.repData?.preinspection_yearly_sales?.total_cost || 0))] : []),
      ...(showLoloSurcharge ? [(this.repData?.lolo_yearly_sales?.total_count || 0), Utility.formatNumberDisplay((this.repData?.lolo_yearly_sales?.total_cost || 0))] : []),
      ...(showStorageSurcharge ? [(this.repData?.storage_yearly_sales?.total_count || 0), Utility.formatNumberDisplay((this.repData?.storage_yearly_sales?.total_cost || 0))] : []),
      ...(showSteamSurcharge ? [(this.repData?.steaming_yearly_sales?.total_count || 0), Utility.formatNumberDisplay((this.repData?.steaming_yearly_sales?.total_cost || 0))] : []),
      ...(showCleanSurcharge ? [(this.repData?.cleaning_yearly_sales?.total_count || 0), Utility.formatNumberDisplay((this.repData?.cleaning_yearly_sales?.total_cost || 0))] : []),
      ...(showResidueSurcharge ? [(this.repData?.residue_yearly_sales?.total_count || 0), Utility.formatNumberDisplay((this.repData?.residue_yearly_sales?.total_cost || 0))] : []),
      ...(showRepairSurcharge ? [(this.repData?.repair_yearly_sales?.total_count || 0), Utility.formatNumberDisplay((this.repData?.repair_yearly_sales?.total_cost || 0))] : []),
    ...
    data.push([
      ...
      ...(showGateSurcharge ? [(this.repData?.gate_yearly_sales?.average_count || 0), Utility.formatNumberDisplay((this.repData?.gate_yearly_sales?.average_cost || 0))] : []),
      ...(showPreinspectSurcharge ? [(this.repData?.preinspection_yearly_sales?.average_count || 0), Utility.formatNumberDisplay((this.repData?.preinspection_yearly_sales?.average_cost || 0))] : []),
      ...(showLoloSurcharge ? [(this.repData?.lolo_yearly_sales?.average_count || 0), Utility.formatNumberDisplay((this.repData?.lolo_yearly_sales?.average_cost || 0))] : []),
      ...(showStorageSurcharge ? [(this.repData?.storage_yearly_sales?.average_count || 0), Utility.formatNumberDisplay((this.repData?.storage_yearly_sales?.average_cost || 0))] : []),
      ...(showSteamSurcharge ? [(this.repData?.steaming_yearly_sales?.average_count || 0), Utility.formatNumberDisplay((this.repData?.steaming_yearly_sales?.average_cost || 0))] : []),
      ...(showCleanSurcharge ? [(this.repData?.cleaning_yearly_sales?.average_count || 0), Utility.formatNumberDisplay((this.repData?.cleaning_yearly_sales?.average_cost || 0))] : []),
      ...(showResidueSurcharge ? [(this.repData?.residue_yearly_sales?.average_count || 0), Utility.formatNumberDisplay((this.repData?.residue_yearly_sales?.average_cost || 0))] : []),
      ...(showRepairSurcharge ? [(this.repData?.repair_yearly_sales?.average_count || 0), Utility.formatNumberDisplay((this.repData?.repair_yearly_sales?.average_cost || 0))] : []),

      total_all_cost / avgCounter
    ]);
    ...
    autoTable(pdf, {
      ...
      didParseCell: (hook: any) => {
        ...
        const avgRow = data.length - 1;

        if (hook.section === "body") {

          const isSummary = hook.row.index === totalRow || hook.row.index === avgRow;

          if (isSummary) {

            hook.cell.styles.fontStyle = "bold";
            hook.cell.styles.fillColor = [230, 230, 230];
            hook.cell.styles.valign = "middle";

            // MERGE LABEL CELL (colSpan)
            if (hook.column.index === 0) {
              hook.cell.colSpan = 2;
              hook.cell.styles.halign = "right";
            }

            // APPLY CENTER ALIGN TO VALUE COLUMNS
            if (hook.column.index > 0) {
