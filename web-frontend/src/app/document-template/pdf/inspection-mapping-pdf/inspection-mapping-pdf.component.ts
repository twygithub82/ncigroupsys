...
@Component({
  selector: 'app-inspection-mapping-pdf',
  templateUrl: './inspection-mapping-pdf.component.html',
  styleUrls: ['./inspection-mapping-pdf.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class InspectionMappingPdfComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<InspectionMappingPdfComponent>,
    ...
    this.type = data.type;
    this.inspectDS = data.inspectDS || new InspectionsDS(this.apollo);
  ...
  async ngOnInit() {
    ...
    const data = await this.getInspectionData();
    if (data?.length > 0) {
  ...
  getInspectionData(): Promise<any[]> {
    return new Promise((resolve, reject) => {
      this.subs.sink = this.inspectDS.getInspectionByIDForEirPdf(this.gate_survey_guid!)
        .subscribe({
