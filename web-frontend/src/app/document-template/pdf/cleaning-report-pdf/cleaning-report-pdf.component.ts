...
export class CleanReportPdfComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.cleanGuid) {
      ...
      // this.cleanItem = data[0];
      // this.estimate_no = this.cleanItem?.estimate_no;
      // this.pdfTitle =  this.translatedLangText.CLEANING_REPORT ;
      // await this.getCodeValuesData();
      // this.updateData(this.cleanItem?.repair_part);
      // this.last_test_desc = this.getLastTest(this.cleanItem?.storing_order_tank?.in_gate?.[0]?.in_gate_survey);
  ...
  async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    var data: any[][] = [
      [
        ...
        { content: `${this.cleanItem?.storing_order_tank?.tank_no}` },
  ...
  createRepairEstimateDetail(pdf: jsPDF, startY: number, leftMargin: number, rightMargin: number, pageWidth: number) {
    ...
    var remarksValue = `${this.cleanItem?.remarks}`;
    startY += 4;
  ...
  createSummaryTable(pdf: jsPDF, leftMargin: number, rightMargin: number, pageWidth: number, pageHeight: number) {
    ...
    const data: RowInput[] = [
      ...
      [
        ...
        { content: `$ ${this.parse2Decimal(this.cleanItem?.labour_cost)}`, styles: { halign: 'right', fontStyle: 'bold' } },
      ...
      [
        { content: `${this.translatedLangText.FOR} ${this.cleanItem?.storing_order_tank?.storing_order?.customer_company?.name}`, colSpan: 1, rowSpan: 3, styles: { halign: 'left', fontStyle: 'bold' } },
  ...
  getReportTitle() {
    return this.translatedLangText.CERTIFICATE_CLEANILINESS;
  }
...
async AddCleaningStepsTable(
 ...
 async AddCleaningDetailsTable(pdf: jsPDF, pageWidth: number, leftMargin: number, rightMargin: number,posY:number):Promise<number>
      ...
      for (const { page, x, y } of pagePositions) {
        ...
        if (page==1)
        {
         ...
         if(page==totalPages)
          ...
          if(this.cleanItem.owner_enable)
          {
           var buffer=3;
  ...
  .reduce((sum, col: any) => sum + (col.cellWidth || 0), 0);
    ...
    rows.push([
     ...
     if(this.cleanItem.storing_order_tank?.customer_company?.currency?.currency_code != systemCurrencyCode){
      var netCost = Number(this.getNetCost().replace(",",""));
      var rate = this.cleanItem.storing_order_tank?.customer_company?.currency?.rate || 1;
      var foreignNetCost = netCost * rate;
      rows.push([
        { content: `${this.translatedLangText.NET_COST}(${this.cleanItem.storing_order_tank?.customer_company?.currency?.currency_code})`, styles: { halign: 'right', fontStyle: fontStyle,fontSize:(fontSz+1) } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
  ...
  .reduce((sum, col: any) => sum + (col.cellWidth || 0), 0);
    ...
    if(this.cleanItem.storing_order_tank?.customer_company?.currency?.currency_code != systemCurrencyCode){
      ...
      rows.push([
        { content: `${this.translatedLangText.NET_COST}(${this.cleanItem.storing_order_tank?.customer_company?.currency?.currency_code})`, styles: { halign: 'right', fontStyle: fontStyle } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
  ...
  .reduce((sum, col: any) => sum + (col.cellWidth || 0), 0);
    ...
    rows.push([
        { content: this.cleanItem.storing_order_tank.customer_company.name, rowSpan:4, styles: { halign: 'left', valign:'bottom', fontStyle: 'bold' } },
 ...
 if(this.cleanItem.storing_order_tank?.customer_company?.currency?.currency_code != systemCurrencyCode){
      ...
      rows.push([
        { content: `${this.translatedLangText.NET_COST}(${this.cleanItem.storing_order_tank?.customer_company?.currency?.currency_code})`, styles: { halign: 'center', fontStyle: fontStyle } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
  ...
  .reduce((sum, col: any) => sum + (col.cellWidth || 0), 0);
    ...
    rows.push([
     ...
     if(this.cleanItem.storing_order_tank?.customer_company?.currency?.currency_code != systemCurrencyCode){
      ...
      var foreignNetCost = netCost * rate;
      rows.push([
        { content: `${this.translatedLangText.EQUIVALENT}(${this.cleanItem.storing_order_tank?.customer_company?.currency?.currency_code})`, styles: { halign: 'right', fontStyle: 'bold',fontSize:(fontSz) } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
  ...
  .reduce((sum, col: any) => sum + (col.cellWidth || 0), 0);
    ...
    if(this.cleanItem.storing_order_tank?.customer_company?.currency?.currency_code != systemCurrencyCode){
      ...
      var foreignLesseeNetCost = lesseeNetCost * rate;
      rows.push([
        { content: `${this.translatedLangText.NET_COST}(${this.cleanItem.storing_order_tank?.customer_company?.currency?.currency_code})`, styles: { halign: 'right', fontStyle: 'bold',fontSize:(fontSz) } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
        { content: '', styles: { halign: 'center', fontStyle: fontStyle } },
    ...
    getLabourCost(ApprovedValue:boolean=false): string{
       var isApproved= BusinessLogicUtil.isEstimateApproved(this.cleanItem);
       if((isApproved && ApprovedValue)||(!isApproved && !ApprovedValue))
       {
        return Utility.formatNumberDisplay(BusinessLogicUtil.roundUpCost(this.cleanItem?.labour_cost));
       }
       else 
       {
          return '';
    ...
    // }
     ...
     getLabourDiscount(): string{
      return this.cleanItem?.labour_cost_discount;
    }

    getMaterialDiscount(): string{
      return this.cleanItem?.material_cost_discount;
    }
    ...
    //   }
      ...
      getSurveyorName()
      {
        return this.cleanItem?.aspnetsuser?.userName||'';
      }

      getLabourRate()
      {
        return  Utility.formatNumberDisplay(this.cleanItem?.labour_cost||0);
      }
