...
export interface DialogData {
  repData: CustomerCompanyCleaningCategoryItem[],
  date: string
}
...
@Component({
  selector: 'app-package-cleaning-cost-report-pdf',
  templateUrl: './package-cleaning-cost-pdf.component.html',
  styleUrls: ['./package-cleaning-cost-pdf.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class PackageCleaningCostPdfComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<PackageCleaningCostPdfComponent>,
   ...
   async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    const data: any[][] = items.map((item,index) => {
      const row = [
        ...
        this.parse2Decimal(item.cleaning_category?.cost!) || "-",
    ...
    const comStyles: any = {
      ...
      1: { cellWidth: 42 ,valign: 'middle', halign: 'left'},   // "NAME"
 ...
 async export(items: TariffCleaningItem[]) {
  ...
  GetReportTitle(): string {
    var title: string = '';
     title = `${this.translatedLangText.PACKAGE_CLEANING}`;
    // switch (this.repType) {
    //   case "CLEANING":
