...
@Component({
  selector: 'app-customer-invoices-excel',
  templateUrl: './customer-invoices-excel.component.html',
  styleUrls: ['./customer-invoices-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class CustomerInvoicesExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<CustomerInvoicesExcelComponent>,
 ...
 async exportToExcel_r1(fileName: string = 'document.xlsx') {
  ...
  for (const cust of this.repBillingCustomers) {
    ...
    // Customer title row
    wsData.push([cust.customer]);

    // Header PER CUSTOMER
    wsData.push(headers);

    if (cust.items!.length > 0) {
      cust.items!.forEach((itm: any, idx: number) => {
        wsData.push([
          idx + 1,
          itm.tank_no || "",
          itm.eir_no || "",
    ...
    wsData.push([]); // spacing between customers
  }
  ...
  for (let i = 0; i < wsData.length; i++) {
    if (JSON.stringify(wsData[i]) === JSON.stringify(headers)) {
      headerRows.push(i);
    }
  }
  ...
  for (let R = range.s.r; R <= range.e.r; R++) {
    for (let C = range.s.c; C <= range.e.c; C++) {
      ...
      // Customer row (only first column)
      if (typeof wsData[R]?.[0] === "string" &&
          wsData[R]?.length === 1 &&
          C === 0 &&
          R > 2) {
  ...
  async exportToPDF_r1_b(fileName: string = 'document.pdf') {
    const pageWidth = 297; // A4 width in mm (landscape)
    ...
    let fontSize = 6;
    let invPeriod = this.repBillingCustomers?.[0].invoice_period;
    lastTableFinalY += 8;
    pdf.setFontSize(8);
    ...
    for (let n = 0; n < this.repBillingCustomers.length; n++) {
      ...
      var tableHeight = ((cust.items?.length || 0) * tableRowHeight + tableHeaderHeight); // Approximate table height
