...
@Component({
  selector: 'app-pending-estimate-report-excel',
  templateUrl: './pending-estimate-report-excel.component.html',
  styleUrls: ['./pending-estimate-report-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class PendingEstimateReportExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<PendingEstimateReportExcelComponent>,
  ...
  async exportToExcel_r1(fileName: string = 'document.xlsx') {
    ...
    for (let n = 0; n < this.sotList.length; n++) {
      const itm = this.sotList?.[n]!;

      data.push([
        (++idx).toString(),
        itm.tank_no || "",
        this.DisplayCustomerCode(itm) || "",
        this.DisplayEIRNo(itm) || "",
        this.DisplayEIRDate(itm) || "",
        this.DisplayCleanIn(itm) || "",
        this.DisplayCleanEnd(itm) || "",
        this.DisplayRepairType(itm) || "",
        this.DisplayDays(itm) || "0"
      ]);
    }
  ...
  async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    for (let n = 0; n < this.sotList.length; n++) {
      ...
      data.push([
        ...
        this.DisplayEIRDate(itm) || "", this.DisplayCleanIn(itm) || "", this.DisplayCleanEnd(itm) || "",
