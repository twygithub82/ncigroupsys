...
@Component({
  selector: 'app-cleaning-detail-excel',
  templateUrl: './cleaning-detail-excel.component.html',
  styleUrls: ['./cleaning-detail-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class CleaningDetailInventoryExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<CleaningDetailInventoryExcelComponent>,
  ...
  const headerRow = [
    ...
    this.translatedLangText.CLEAN_END,
  ...
  for (let n = 0; n < this.report_inventory_cln_dtl.length; n++) {
    ...
    // 🔹 Rows
    sectionRows.forEach(r => data.push(r));

    // 🔹 Spacer row
    data.push([]);
  }

  // 🔹 Global merges (title + subtitle)
  merges.push(
    { s: { r: 0, c: 0 }, e: { r: 0, c: 5 } },
    { s: { r: 1, c: 0 }, e: { r: 1, c: 5 } }
  );
  ...
  for (let R = range.s.r; R <= range.e.r; ++R) {
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      // 🔹 Data rows
      if (R > 2 && cell.v !== undefined && !String(cell.v).includes('|')) {
        cell.s = {
          font: { sz: 10 },
