...
@Component({
  selector: 'app-unclean-tank-detail-excel',
  templateUrl: './unclean-tank-excel.component.html',
  styleUrls: ['./unclean-tank-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class UncleanTankDetailInventoryExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<UncleanTankDetailInventoryExcelComponent>,
  ...
  const cutoffDate = PDFUtility.FormatColon(
    this.translatedLangText.CLEANING_PERIOD,
    this.date
  );
  ...
  const headerRow = [
    ...
    this.translatedLangText.CLEAN_IN,
    this.translatedLangText.DURATION_DAYS
  ];

  let merges: XLSX.Range[] = [];

  // 🔹 Loop sections (same as PDF)
  for (let n = 0; n < this.report_inventory_cln_dtl.length; n++) {
    ...
    const sectionStartRow = data.length;

    // 🔹 Build rows
    for (let i = 0; i < (cust.storing_order_tank?.length || 0); i++) {
      let itm = cust.storing_order_tank?.[i];

      data.push([
        (i + 1).toString(),
        itm?.tank_no || "",
        this.DisplayCustomerName(itm!) || "",
        this.DisplayCleanIn(itm!) || "",
        this.DipslayCleanDuration(itm!) || ""
      ]);
    ...
    // 🔹 Insert subtitle above section
    const subtitle = `${cust.cargo || "-"} | ${unNo || "-"} | ${process || "-"}`;
    data.splice(sectionStartRow, 0, [subtitle]);

    // 🔹 Insert header
    data.splice(sectionStartRow + 1, 0, headerRow);

    // 🔹 Merge subtitle across columns
    merges.push({
      s: { r: sectionStartRow, c: 0 },
      e: { r: sectionStartRow, c: 4 }
    });
  ...
  for (let R = range.s.r; R <= range.e.r; ++R) {
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      // 🔹 Section subtitle (Cargo | UN | Process)
      ...
      if (
        cell.v === this.translatedLangText.S_N ||
        cell.v === this.translatedLangText.TANK_NO
      ) {
        cell.s = {
          font: { bold: true },
          alignment: center,
      ...
      // 🔹 Data rows
      if (R > 2 && typeof cell.v !== "undefined") {
        cell.s = {
          font: { sz: 10 },
  ...
  async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    const headers = [[
      this.translatedLangText.S_N,
      this.translatedLangText.TANK_NO, this.translatedLangText.CUSTOMER,
      this.translatedLangText.CLEAN_IN, this.translatedLangText.DURATION_DAYS, //this.translatedLangText.UN_NO,
    ]];
    ...
    //  startY = lastTableFinalY + 8;
      ...
      for (let i = 0; i < (cust.storing_order_tank?.length || 0); i++) {
        ...
        data.push([
          (i + 1).toString(), itm?.tank_no || "", this.DisplayCustomerName(itm!) || "", this.DisplayCleanIn(itm!) || "", 
  ...
  DipslayCleanDuration(sot: StoringOrderTankItem) {
    ...
    const end_dt = Math.floor(Date.now() / 1000);
