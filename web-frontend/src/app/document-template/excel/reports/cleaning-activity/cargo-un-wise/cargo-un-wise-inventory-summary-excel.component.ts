...
@Component({
  selector: 'app-cargo-un-wise-inventory-summary-excel',
  templateUrl: './cargo-un-wise-inventory-summary-excel.component.html',
  styleUrls: ['./cargo-un-wise-inventory-summary-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class CargoUNWiseInventorySummaryExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<CargoUNWiseInventorySummaryExcelComponent>,
  ...
  ws['!merges'] = [
    ...
    { s: { r: totalRowIndex, c: 0 }, e: { r: totalRowIndex, c: 1 } } // TOTAL merge
  ];
  ...
  for (let R = range.s.r; R <= range.e.r; ++R) {
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      if (R === totalRowIndex) {
        ws[cellRef].s = {
          font: { bold: true },
          alignment: C === 0 ? rightAlign : centerAlign,
