...
@Component({
  selector: 'app-transfer-location-excel',
  templateUrl: './transfer-location-excel.component.html',
  styleUrls: ['./transfer-location-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class TransferLocationExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<TransferLocationExcelComponent>,
  ...
  async exportToExcel_r1(fileName: string = 'Transfer_Report.xlsx') {
  ...
  const period = PDFUtility.FormatColon(
    this.translatedLangText.TRANSFER_PERIOD,
    this.date
  );
  ...
  for (let n = 0; n < this.report_transfer_location.length; n++) {
    ...
    const cust = this.report_transfer_location[n];

    // ===== CUSTOMER TITLE =====
    XLSX.utils.sheet_add_aoa(ws, [[cust.customer || ""]], { origin: `A${rowIndex + 1}` });
    ws[`A${rowIndex + 1}`].s = {
      font: { bold: true }
    };
    rowIndex++;
    ...
    for (let i = 0; i < (cust.storing_order_tank?.length || 0); i++) {
      const itm = cust.storing_order_tank?.[i]!;

      data.push([
        (i + 1).toString(),
        itm.tank_no || "",
        this.DisplayEIRNo(itm) || "",
        this.DisplayEIRDateIn(itm) || "",
        this.DisplayEIRDateOut(itm) || "",
        this.DisplayFromYard(itm) || "",
        this.DisplayToYard(itm) || "",
        this.DisplayDateIn(itm) || "",
        this.DisplayDateOut(itm) || ""
      ]);
    }
    ...
    rowIndex += data.length + 2; // spacing
  }
