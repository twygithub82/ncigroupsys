...
import { report_customer_inventory, periodic_test_due_item, report_periodic_test_due_group_customer } from 'app/data-sources/reports';
...
export interface DialogData {
  report_inventory: report_periodic_test_due_group_customer[],
  date: string
}

@Component({
  selector: 'app-periodic-test-excel',
  templateUrl: './periodic-test-excel.component.html',
  styleUrls: ['./periodic-test-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class PeriodicTestDueExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  report_customer_inventory: report_periodic_test_due_group_customer[] = [];
  date: string = '';
  invType: string = '';
  queryType: number = 1;
  ...
  constructor(
    public dialogRef: MatDialogRef<PeriodicTestDueExcelComponent>,
  ...
  async exportToExcel_r1(fileName: string = 'periodic-test.xlsx') {
  ...
  const headerRow1Index = data.length;
  ...
  data.push([
    ...
    this.translatedLangText.LAST_PERIODIC_TEST,
    "", "",
    this.translatedLangText.NEXT_PERIODIC_TEST,
    "",
    this.translatedLangText.DUES,
    this.translatedLangText.DUE_TYPE
  ]);

  // 🔹 HEADER ROW 2 (Sub header)
  const headerRow2Index = data.length;

  data.push([
    "", "", "", "", "",
    this.translatedLangText.TYPE,
    this.translatedLangText.DATE,
    this.translatedLangText.CLASS,
    this.translatedLangText.TYPE,
    this.translatedLangText.DATE,
    "", ""
  ]);
  ...
  merges.push(
    // RowSpan (vertical)
    { s: { r: headerRow1Index, c: 0 }, e: { r: headerRow2Index, c: 0 } },
    { s: { r: headerRow1Index, c: 1 }, e: { r: headerRow2Index, c: 1 } },
    { s: { r: headerRow1Index, c: 2 }, e: { r: headerRow2Index, c: 2 } },
    { s: { r: headerRow1Index, c: 3 }, e: { r: headerRow2Index, c: 3 } },
    { s: { r: headerRow1Index, c: 4 }, e: { r: headerRow2Index, c: 4 } },
    { s: { r: headerRow1Index, c: 10 }, e: { r: headerRow2Index, c: 10 } },
    { s: { r: headerRow1Index, c: 11 }, e: { r: headerRow2Index, c: 11 } },

    // ColSpan (horizontal)
    { s: { r: headerRow1Index, c: 5 }, e: { r: headerRow1Index, c: 7 } },
    { s: { r: headerRow1Index, c: 8 }, e: { r: headerRow1Index, c: 9 } }
  );
  ...
  for (let n = 0; n < this.report_customer_inventory.length; n++) {
    ...
    // 🔹 Customer title
    const custRow = data.length;
    data.push([this.DisplayCustomerName(cust) || "-"]);
    ...
    for (let b = 0; b < (cust.periodic_test_due?.length || 0); b++) {
      ...
      let itm = cust.periodic_test_due?.[b];

      data.push([
        (b + 1).toString(),
        itm!.tank_no || "",
        this.DisplayEIRNo(itm!) || "",
        this.DisplayEIRDate(itm!) || "",
        this.DisplayOwner(itm!) || "",
        this.DisplayLastTest(itm!) || "",
        this.DisplayLastTestDate(itm!) || "",
        this.DisplayClass(itm!) || "",
        this.DisplayNextTest(itm!) || "",
        this.DisplayNextTestDate(itm!) || "",
        this.DisplayDueDays(itm!) || "0",
        this.DisplayDueType(itm!) || ""
      ]);
    }
  ...
  for (let R = range.s.r; R <= range.e.r; ++R) {
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      // 🔹 Header rows
      if (R === headerRow1Index || R === headerRow2Index) {
        cell.s = {
          font: { bold: true },
          alignment: center,
      ...
      // 🔹 Customer row
      if (C === 0 && merges.some(m => m.s.r === R && m.e.c === 11)) {
        cell.s = {
          font: { bold: true },
          alignment: left
        };
   ...
   async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    const headers = [
      ...
      [
        ...
        this.translatedLangText.TYPE, this.translatedLangText.DATE, this.translatedLangText.CLASS, // Sub-headers for LAST_PERIODIC_TEST
        this.translatedLangText.TYPE, this.translatedLangText.DATE, // Sub-headers for NEXT_PERIODIC_TEST
    ...
    for (let n = 0; n < this.report_customer_inventory.length; n++) {
     ...
     // lastTableFinalY += 7;
      ...
      if ((cust.periodic_test_due?.length || 0) > 0) {
        ...
        for (let b = 0; b < (cust.periodic_test_due?.length || 0); b++) {
          var itm = cust.periodic_test_due?.[b]!;
          data.push([
  ...
  async exportToPDF_r1_old(fileName: string = 'document.pdf') {
    ...
    pagePositions.forEach(({ page, x, y }) => {
      ...
      pdf.line(leftMargin, pdf.internal.pageSize.height - lineBuffer, (pageWidth - rightMargin - 1), pdf.internal.pageSize.height - lineBuffer);
    });
