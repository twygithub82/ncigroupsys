...
import { report_status, report_customer_tank_activity } from 'app/data-sources/reports';
...
export interface DialogData {
  report_yard_detail: report_status[],

}

@Component({
  selector: 'app-yard-detail-excel',
  templateUrl: './yard-detail-excel.component.html',
  styleUrls: ['./yard-detail-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class YardDetailInventoryExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<YardDetailInventoryExcelComponent>,
  ...
  async ngOnInit() {
    ...
    this.indexCounter = 0;
    this.report_yard_detail = this.data.report_yard_detail;
  ...
  public loadData(dataDlg: DialogData) {
    ...
    this.cvDS.connectAlias('yardCv').subscribe(data => {
      if (data.length) {
        ...
        this.indexCounter = 0;
        this.report_yard_detail = dataDlg.report_yard_detail;
  ...
  async exportToExcel_r1(fileName: string = 'yard-report.xlsx') {
  ...
  const headerRow = [
    ...
    this.translatedLangText.TAKE_IN_REFERENCE,
  ...
  for (let n = 0; n < this.report_yard_detail.length; n++) {
    ...
    for (let b = 0; b < (cust.yards?.length || 0); b++) {
      ...
      for (let i = 0; i < (y.storing_order_tank?.length || 0); i++) {

        let itm = y.storing_order_tank?.[i];

        data.push([
          (++idx).toString(),
          itm?.tank_no || "",
          this.DisplayLocation(y.code!) || "",
          this.DisplayEIRNo(itm!) || "",
          this.DisplayOwner(itm!) || "",
          this.DisplayInDate(itm!) || "",
          this.DisplayTakeInRef(itm!) || "",
          itm?.tariff_cleaning?.cargo || "",
          this.DisplayCleanDate(itm!) || "",
          this.DisplayEstimateNo(itm!) || "",
          this.DisplayEstimateDate(itm!) || "",
          this.DisplayApprovalDate(itm!) || "",
          this.DisplayAVDate(itm!) || "",
          this.DisplayLastTest(itm!) || "",
          this.DisplayNextTest(itm!) || "",
          this.DisplayCurrentStatus(itm!) || "",
          this.DisplayRemarks(itm!) || "",
  ...
  // 🔹 Column widths (IMPORTANT for wide layout)
  ws['!cols'] = [
    { wch: 5 },   // S/N
    { wch: 18 },  // Tank No
    { wch: 10 },
  ...
  for (let R = range.s.r; R <= range.e.r; ++R) {
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      // 🔹 Customer row
      if (C === 0 && merges.some(m => m.s.r === R && m.e.c === 17)) {
        cell.s = {
          font: { bold: true },
          alignment: left
        };
      ...
      // 🔹 Header row
      if (cell.v === this.translatedLangText.S_N) {
        for (let c = 0; c < 18; c++) {
          const hRef = XLSX.utils.encode_cell({ r: R, c });
          if (ws[hRef]) {
            ws[hRef].s = {
              font: { bold: true },
              alignment: center,
      ...
      cell.s = {
        font: { sz: 10 },
        alignment: (C === 1 || C === 7 || C === 16) ? left : center,
        border: border
      };
    }
  }
  ...
  async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    const headers = [[
      ...
      this.translatedLangText.NEXT_TEST, this.translatedLangText.CURRENT_STATUS,
    ...
    const comStyles: any = {
      ...
      15: { halign: 'center', valign: 'middle', cellWidth: 17, minCellHeight: minHeightBodyCell },
    ...
    for (let n = 0; n < this.report_yard_detail.length; n++) {
     ...
     lastTableFinalY+=PDFUtility.GapBetweenLeftTitleAndTable();
      if ((cust.yards?.length || 0) > 0) {
        ...
        for (let b = 0; b < (cust.yards?.length || 0); b++) {
          var y = cust.yards?.[b]!;
          for (let n = 0; n < (y.storing_order_tank?.length || 0); n++) {
            var itm = y.storing_order_tank?.[n];
            data.push([
              (++idx).toString(), itm?.tank_no || "", this.DisplayLocation(y.code!) || "", this.DisplayEIRNo(itm!) || "",
              this.DisplayOwner(itm!) || "", this.DisplayInDate(itm!) || "", this.DisplayTakeInRef(itm!) || "",
  ...
  DisplayLocation(yardCv: string) {
    return this.cvDS.getCodeDescription(yardCv, this.yardCvList);
  }
}
