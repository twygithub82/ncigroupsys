...
@Component({
  selector: 'app-tank-survey-excel',
  templateUrl: './tank-survey-excel.component.html',
  styleUrls: ['./tank-survey-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class TankSurveyExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  langText = {
    ...
    TANK_SURVEY_REPORT: 'COMMON-FORM.DAILY-TANK-SURVEY',
  ...
  constructor(
    public dialogRef: MatDialogRef<TankSurveyExcelComponent>,
  ...
  async exportToExcel_r1(fileName: string = 'Survey_Report.xlsx') {
  ...
  const period = `${this.translatedLangText.SURVEY_PERIOD}: ${this.date}`;
  ...
  // ================= SUBTITLE =================
  XLSX.utils.sheet_add_aoa(ws, [[period]], { origin: `A${rowIndex + 1}` });
  ws[`A${rowIndex + 1}`].s = {
    alignment: { horizontal: 'center' }
  };
  merges.push({ s: { r: rowIndex, c: 0 }, e: { r: rowIndex, c: 6 } });
  ...
  for (let n = 0; n < this.report_tank_summaries.length; n++) {
    ...
    const cust = this.report_tank_summaries[n];

    // ===== SURVEY DATE TITLE =====
    const surveyText = `${this.translatedLangText.SURVEY_DATE}: ${cust.survey_dt}`;
    XLSX.utils.sheet_add_aoa(ws, [[surveyText]], { origin: `A${rowIndex + 1}` });
    ...
    for (let i = 0; i < (cust.tank_survey_summaries?.length || 0); i++) {
      const itm = cust.tank_survey_summaries?.[i]!;
      ...
      data.push([
        ...
        itm.tank_no || "",
        itm.eir_no || "",
        this.DisplaySurveyType(itm.survey_type!) || "",
        this.DisplaySurveyStatus(itm.status!) || "",
        itm.surveryor || ""
      ]);
    }
    ...
    // ===== STYLE BODY =====
    for (let r = rowIndex; r < rowIndex + data.length; r++) {
      for (let c = 0; c < headers.length; c++) {
    ...
    rowIndex += data.length + 2; // spacing between groups
  }
  ...
  async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    const comStyles: any = {
      ...
      4: { halign: 'center', valign: 'middle', cellWidth: 43, minCellHeight: minHeightBodyCell },
  ...
  DisplaySurveyType(surveyType: string): string {

    return this.cvDS.getCodeDescription(surveyType, this.surveyTypeCvList) || '';;
  }

  DisplaySurveyStatus(surveyStatus: string): string {

    return this.cvDS.getCodeDescription(surveyStatus, this.surveyStatusCvList) || '';;
  }
