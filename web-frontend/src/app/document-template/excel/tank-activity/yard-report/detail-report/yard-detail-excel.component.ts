...
export class YardDetailExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<YardDetailExcelComponent>,
  ...
  exportToExcel_r1(fileName: string = 'document.xlsx') {
    ...
    const headers = [
      ...
      this.translatedLangText.TARE_WEIGHT,
      ...
      this.translatedLangText.APPROVAL_REFERENCE,
      this.translatedLangText.AV_DATE,
      this.translatedLangText.LAST_TEST,
      this.translatedLangText.RELEASE_DATE,
      this.translatedLangText.RELEASE_REFERENCE,
    ...
    for (let n = 0; n < this.report_customer_tank_activity.length; n++) {
      ...
      if ((cust.storing_order_tank?.length || 0) > 0) {
        ...
        for (let b = 0; b < (cust.storing_order_tank?.length || 0); b++) {
          ...
          const itm = cust.storing_order_tank?.[b]!;
          ...
          wsData.push([
            ...
            this.DisplayCapacity(itm) || "",
            this.DisplayTareWeight(itm) || "",
            ...
            this.DisplayApprovalRef(itm) || "",
            this.DisplayAVDate(itm) || "",
            this.DisplayLastTest(itm) || "",
            this.DisplayReleaseDate(itm) || "",
            this.DisplayReleaseRef(itm) || "",
            this.DisplayCurrentStatus(itm) || "",
            this.DisplayRemarks(itm) || "",
            this.displayTankPurpose(itm) || ""
          ]);

          // Additional repair rows
          if (itm.repair && (itm.repair.length || 0) > 1) {

            for (let r = 1; r < itm.repair.length; r++) {

              const rp = itm.repair[r];
    ...
    for (let R = 0; R <= range.e.r; ++R) {
      ...
      for (let C = 0; C <= range.e.c; ++C) {
        ...
        // TABLE HEADER STYLE
        // =====================================
        if (row[0] === this.translatedLangText.S_N) {
        ...
        if (
          typeof row[0] === 'string' &&
          row[0].startsWith('##CUSTOMER##')
        ) {

          ws[cellRef].v = row[0].replace('##CUSTOMER##', '');
