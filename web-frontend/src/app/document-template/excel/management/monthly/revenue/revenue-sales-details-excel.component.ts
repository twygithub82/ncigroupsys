...
@Component({
  selector: 'app-monthly-revenue-sales-report-details-excel',
...
export class RevenueMonthlySalesReportDetailsExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<RevenueMonthlySalesReportDetailsExcelComponent>,
  ...
  async exportToExcel_r1(fileName: string = 'report.xlsx') {
    ...
    if (showGateSurcharge) pushHeader(this.translatedLangText.GATE);
    ...
    const grpData = InventoryAnalyzer.groupRevenueMonthlyByDate(this.repData!);
    ...
    for (const date in grpData) {
      ...
      data.push([
        ...
        ...(showPreinspectSurcharge ? [entry.preinspection?.count || 0, entry.preinspection?.cost || 0] : []),
        ...(showLoloSurcharge ? [entry.lolo?.count || 0, entry.lolo?.cost || 0] : []),
        ...(showStorageSurcharge ? [entry.storage?.count || 0, entry.storage?.cost || 0] : []),
        ...(showGateSurcharge ? [entry.gate?.count || 0, entry.gate?.cost || 0] : []),
        ...(showSteamSurcharge ? [entry.steaming?.count || 0, entry.steaming?.cost || 0] : []),
        ...(showResidueSurcharge ? [entry.residue?.count || 0, entry.residue?.cost || 0] : []),
        ...(showCleanSurcharge ? [entry.cleaning?.count || 0, entry.cleaning?.cost || 0] : []),
        ...(showRepairSurcharge ? [entry.repair?.count || 0, entry.repair?.cost || 0] : [])
      ]);
    }

    const buildRow = (label: string, type: 'total' | 'average') => {
      const row: any[] = [label, "", ""];

      const getVal = (obj: any, key: string) => obj?.[key] || 0;

      if (showPreinspectSurcharge) row.push(getVal(this.repData?.preinspection_monthly_revenue, `${type}_count`), getVal(this.repData?.preinspection_monthly_revenue, `${type}_cost`));
      if (showLoloSurcharge) row.push(getVal(this.repData?.lolo_monthly_revenue, `${type}_count`), getVal(this.repData?.lolo_monthly_revenue, `${type}_cost`));
      if (showStorageSurcharge) row.push(getVal(this.repData?.storage_monthly_revenue, `${type}_count`), getVal(this.repData?.storage_monthly_revenue, `${type}_cost`));
      if (showGateSurcharge) row.push(getVal(this.repData?.gate_monthly_revenue, `${type}_count`), getVal(this.repData?.gate_monthly_revenue, `${type}_cost`));
      if (showSteamSurcharge) row.push(getVal(this.repData?.steam_monthly_revenue, `${type}_count`), getVal(this.repData?.steam_monthly_revenue, `${type}_cost`));
      if (showResidueSurcharge) row.push(getVal(this.repData?.residue_monthly_revenue, `${type}_count`), getVal(this.repData?.residue_monthly_revenue, `${type}_cost`));
      if (showCleanSurcharge) row.push(getVal(this.repData?.cleaning_monthly_revenue, `${type}_count`), getVal(this.repData?.cleaning_monthly_revenue, `${type}_cost`));
      if (showRepairSurcharge) row.push(getVal(this.repData?.repair_monthly_revenue, `${type}_count`), getVal(this.repData?.repair_monthly_revenue, `${type}_cost`));

      return row;
    };

    data.push(buildRow(this.translatedLangText.TOTAL, 'total'));
    data.push(buildRow(this.translatedLangText.AVERAGE, 'average'));
    ...
    const colCount = headerRow1.length;
    ...
    for (let C = 0; C <= range.e.c; C++) {
      let h1 = ws[XLSX.utils.encode_cell({ r: 3, c: C })];
      let h2 = ws[XLSX.utils.encode_cell({ r: 4, c: C })];

      if (h1) h1.s = headerStyle;
      if (h2) h2.s = headerStyle;
    }
    ...
    if (showPreinspectSurcharge) mergeHeader();
    if (showLoloSurcharge) mergeHeader();
    if (showStorageSurcharge) mergeHeader();
    if (showGateSurcharge) mergeHeader();
    if (showSteamSurcharge) mergeHeader();
    if (showResidueSurcharge) mergeHeader();
    if (showCleanSurcharge) mergeHeader();
    if (showRepairSurcharge) mergeHeader();
    ...
    const fName = this.GetReportTitle() + '.xlsx';
