...
import { ManagementReportYearlyRevenueItem, MonthlyProcessData, InventoryAnalyzer, ManagementReportMonthlyInventory } from 'app/data-sources/reports-management';
import {
  ChartComponent,
  ApexAxisChartSeries,
  ApexChart,
...
export interface DialogData {
  repData: ManagementReportMonthlyInventory,
  date: string,
  repType: string,
  customer: string,
...
@Component({
  selector: 'app-monthly-inventory-sales-report-details-excel',
...
export class InventoryMonthlySalesReportDetailsExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<InventoryMonthlySalesReportDetailsExcelComponent>,
  ...
  async exportToExcel_r1(fileName: string = 'report.xlsx') {
    ...
    merges.push({ s: { r: rowIndex, c: 0 }, e: { r: rowIndex, c: 10 } });
    rowIndex++;
    ...
    const pushHeader = (text: string, colSpan = 1, rowSpan = 1) => {
      header1[col] = text;
    ...
    // Fixed columns
    pushHeader(this.translatedLangText.S_N, 1, 2);
    pushHeader(this.translatedLangText.DATE, 1, 2);
    pushHeader(this.translatedLangText.DAY, 1, 2);

    // Dynamic sections
    if (showCleanSurcharge) {
      pushHeader(this.translatedLangText.CLEANING, 2);
      header2.push(this.translatedLangText.APPROVED_COUNT, this.translatedLangText.COMPLETED_COUNT);
    }

    if (showRepairSurcharge) {
      pushHeader(this.translatedLangText.REPAIR, 2);
      header2.push(this.translatedLangText.APPROVED_HOUR, this.translatedLangText.COMPLETED_HOUR);
    }

    if (showSteamSurcharge) {
      pushHeader(this.translatedLangText.STEAM, 2);
      header2.push(this.translatedLangText.APPROVED_COUNT, this.translatedLangText.COMPLETED_COUNT);
    }

    if (showResidueSurcharge) {
      pushHeader(this.translatedLangText.RESIDUE, 2);
      header2.push(this.translatedLangText.APPROVED_COUNT, this.translatedLangText.COMPLETED_COUNT);
    }
    ...
    // DATA
    // =========================
    const grpData = InventoryAnalyzer.groupInventoryMonthlyByDate(this.repData!);

    let idx = 0;

    let clnApp = 0, clnCmp = 0;
    let repApp = 0, repCmp = 0;
    let stmApp = 0, stmCmp = 0;
    let resApp = 0, resCmp = 0;

    for (const date in grpData) {
      const e = grpData[date];
      ...
      if (showCleanSurcharge) {
        row.push(e.cleaning?.approved_count || 0, e.cleaning?.completed_count || 0);
        clnApp += e.cleaning?.approved_count || 0;
        clnCmp += e.cleaning?.completed_count || 0;
      }

      if (showRepairSurcharge) {
        row.push(e.repair?.approved_hour || 0, e.repair?.completed_hour || 0);
        repApp += e.repair?.approved_hour || 0;
        repCmp += e.repair?.completed_hour || 0;
      }

      if (showSteamSurcharge) {
        row.push(e.steaming?.approved_count || 0, e.steaming?.completed_count || 0);
        stmApp += e.steaming?.approved_count || 0;
        stmCmp += e.steaming?.completed_count || 0;
      }

      if (showResidueSurcharge) {
        row.push(e.residue?.approved_count || 0, e.residue?.completed_count || 0);
        resApp += e.residue?.approved_count || 0;
        resCmp += e.residue?.completed_count || 0;
      }
    ...
    if (showCleanSurcharge) totalRow.push(clnApp, clnCmp);
    if (showRepairSurcharge) totalRow.push(repApp, repCmp);
    if (showSteamSurcharge) totalRow.push(stmApp, stmCmp);
    if (showResidueSurcharge) totalRow.push(resApp, resCmp);

    data.push(totalRow);
    rowIndex++;

    // =========================
    // DIFFERENCE ROW (MERGED)
    // =========================
    let diffRow: any[] = [this.translatedLangText.DIFFERENCE, "", ""];
    let diffColStart = 3;

    const pushDiff = (val: number) => {
      diffRow[diffColStart] = val;
      merges.push({
        s: { r: rowIndex, c: diffColStart },
        e: { r: rowIndex, c: diffColStart + 1 }
      });
      diffColStart += 2;
    };

    if (showCleanSurcharge) pushDiff(clnApp - clnCmp);
    if (showRepairSurcharge) pushDiff(repApp - repCmp);
    if (showSteamSurcharge) pushDiff(stmApp - stmCmp);
    if (showResidueSurcharge) pushDiff(resApp - resCmp);
