...
@Component({
  selector: 'app-yearly-revenue-sales-report-details-excel',
  templateUrl: './revenue-sales-details-excel.component.html',
  styleUrls: ['./revenue-sales-details-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class RevenueYearlySalesReportDetailsExcelComponent extends UnsubscribeOnDestroyAdapter implements AfterViewInit {
  ...
  colors?: [
    ...
    "#ce6dbd", "#e7969c", "#c7c7c7", "#dbdb8d", "#9edae5",
    "#6baed6", "#74c476", "#fd8d3c", "#fdae6b", "#c49c94"
  ]
  // date:string='';
  // invType:string='';
  ...
  constructor(
    public dialogRef: MatDialogRef<RevenueYearlySalesReportDetailsExcelComponent>,
 ...
 async exportToExcel_r1(fileName: string = "document.xlsx") {
  ...
  const showPreinspectSurcharge = this.invTypes?.includes("PREINSPECTION");
  const showLoloSurcharge = this.invTypes?.includes("LOLO");
  const showStorageSurcharge = this.invTypes?.includes("STORAGE");
  ...
  const grpData = InventoryAnalyzer.groupByMonthAndFindExtremesRevenue(this.repData!);
  ...
  const mergePairs: { start: number; end: number }[] = [];

  const pushHeader = (label: string, show: boolean) => {
    if (show) {
      const start = header1.length;

      header1.push(label, "");   // COLSPAN 2
      header2.push(this.translatedLangText.QTY, this.translatedLangText.COST);

      const end = header1.length - 1;

      mergePairs.push({ start, end });
    }
  };

  pushHeader(this.translatedLangText.PREINSPECTION, showPreinspectSurcharge!);
  pushHeader(this.translatedLangText.LOLO, showLoloSurcharge!);
  pushHeader(this.translatedLangText.STORAGE, showStorageSurcharge!);
  pushHeader(this.translatedLangText.GATE_SURCHARGE, showGateSurcharge!);
  pushHeader(this.translatedLangText.STEAM, showSteamSurcharge!);
  pushHeader(this.translatedLangText.RESIDUE, showResidueSurcharge!);
  pushHeader(this.translatedLangText.CLEANING, showCleanSurcharge!);
  pushHeader(this.translatedLangText.REPAIR, showRepairSurcharge!);

  header1.push(this.translatedLangText.TOTAL);
  header2.push("");
  ...
  for (const m of grpData.monthlyData) {
    ...
    const pushCol = (show: boolean, obj: any) => {
      if (show) {
        row.push(obj?.count || 0);
        row.push(obj?.cost || 0);
      }
    };

    pushCol(showPreinspectSurcharge!, m.preinspection);
    pushCol(showLoloSurcharge!, m.lolo);
    pushCol(showStorageSurcharge!, m.storage);
    pushCol(showGateSurcharge!, m.in_out);
    pushCol(showSteamSurcharge!, m.steaming);
    pushCol(showResidueSurcharge!, m.residue);
    pushCol(showCleanSurcharge!, m.cleaning);
    pushCol(showRepairSurcharge!, m.repair);
  ...
  const addTotal = (show: boolean, key: string) => {
    if (show) {
      const data = (this.repData as any)[`${key}_yearly_revenue`];
      totalRow.push("", data?.total_cost || 0);
    }
  };

  addTotal(showPreinspectSurcharge!, "preinspection");
  addTotal(showLoloSurcharge!, "lolo");
  addTotal(showStorageSurcharge!, "storage");
  addTotal(showGateSurcharge!, "gate");
  addTotal(showSteamSurcharge!, "steam");
  addTotal(showResidueSurcharge!, "residue");
  addTotal(showCleanSurcharge!, "cleaning");
  addTotal(showRepairSurcharge!, "repair");

  totalRow.push(total_all_cost);
  ...
  const addAvg = (show: boolean, key: string) => {
    if (show) {
      const data = (this.repData as any)[`${key}_yearly_revenue`];
      avgRow.push("", data?.average_cost || 0);
    }
  };

  addAvg(showPreinspectSurcharge!, "preinspection");
  addAvg(showLoloSurcharge!, "lolo");
  addAvg(showStorageSurcharge!, "storage");
  addAvg(showGateSurcharge!, "gate");
  addAvg(showSteamSurcharge!, "steam");
  addAvg(showResidueSurcharge!, "residue");
  addAvg(showCleanSurcharge!, "cleaning");
  addAvg(showRepairSurcharge!, "repair");

  avgRow.push(total_all_cost / (grpData.monthlyData.length || 1));
  ...
  // =========================
  // HEADER COLSPAN MERGE (ROW 4)
  // =========================
  let col = 2; // after S/N + repType

  mergePairs.forEach(m => {
    ws["!merges"]!.push({
      s: { r: 4, c: col },
      e: { r: 4, c: col + 1 }
    });
  ...
  for (let R = 0; R <= range.e.r; R++) {
    for (let C = 0; C <= range.e.c; C++) {
      ...
      const isTitle = R === 1;
      const isDate = R === 2;
      const isHeader = R === 4 || R === 5;

      cell.s = {
        font: {
          name: "Arial",
          sz: isTitle || isDate ? 10 : isHeader ? 9 : 10,
          bold: isHeader || R >= wsData.length - 2
        },
        alignment: {
          horizontal: "center",
          vertical: "center"
 ...
 this.lineChartOptions.series = series;
    ...
    // setTimeout(async () => {
      ...
      chartContentWidth =chartContentWidth*0.8;
      const cardElements = this.pdfTable.nativeElement.querySelectorAll('.card');
    ...
    this.pieChartOptions = {
      ...
      markers: {
        size: 6,
         ...
         shape: "square",  // optional custom shape (circle/square/rect)
      },
      chart: {
        height: 450,
        type: 'pie',
