...
@Component({
  selector: 'app-yearly-inventory-sales-report-details-excel',
  templateUrl: './inventory-sales-details-excel.component.html',
  styleUrls: ['./inventory-sales-details-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class InventoryYearlySalesReportDetailsExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit, AfterViewInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<InventoryYearlySalesReportDetailsExcelComponent>,
 ...
 async exportToExcel_r1(fileName: string = 'document.xlsx') {
  ...
  const showGateSurcharge = this.invTypes?.includes("IN_OUT")!;
  const showSteamSurcharge = this.invTypes?.includes("STEAMING")!;
  const showCleanSurcharge = this.invTypes?.includes("CLEANING")!;
  const showRepairSurcharge = this.invTypes?.includes("REPAIR")!;
  const showResidueSurcharge = this.invTypes?.includes("RESIDUE")!;

  const reportTitle = this.GetReportTitle();
  const repGeneratedDate = this.date;

  const grpData = InventoryAnalyzer.groupByMonthAndFindExtremes(this.repData!);
  ...
  // HEADER ROW 1
  // =========================
  const header1: any[] = [this.translatedLangText.S_N, this.repType];

  const addHeader = (show: boolean, label: string) => {
    if (show) header1.push(label, "");
  };

  addHeader(showGateSurcharge, this.translatedLangText.IN_GATE);
  addHeader(showGateSurcharge, this.translatedLangText.OUT_GATE);
  addHeader(showSteamSurcharge, this.translatedLangText.STEAM);
  addHeader(showCleanSurcharge, this.translatedLangText.CLEANING);
  addHeader(showRepairSurcharge, this.translatedLangText.REPAIR);
  addHeader(showResidueSurcharge, this.translatedLangText.RESIDUE);
  ...
  const addSub = (show: boolean) => {
    if (show) header2.push("QTY", "PERCENT");
  };

  addSub(showGateSurcharge);
  addSub(showGateSurcharge);
  addSub(showSteamSurcharge);
  addSub(showCleanSurcharge);
  addSub(showRepairSurcharge);
  addSub(showResidueSurcharge);
  ...
  for (const m of grpData.monthlyData) {

    const row: any[] = [++idx, m.key];

    const push = (show: boolean, obj: any, key: keyof typeof totals) => {
      if (show) {
        row.push(obj?.count || 0, obj?.percentage || 0);
        totals[key] += obj?.count || 0;
      }
    };

    push(showGateSurcharge, m.gateIn, "gateIn");
    push(showGateSurcharge, m.gateOut, "gateOut");
    push(showSteamSurcharge, m.steaming, "steam");
    push(showCleanSurcharge, m.cleaning, "clean");
    push(showRepairSurcharge, m.repair, "repair");
    push(showResidueSurcharge, m.residue, "residue");

    wsData.push(row);
  }

  const monthCount = grpData.monthlyData.length || 1;

  // =========================
  // TOTAL ROW
  // =========================
  const totalRow: any[] = [this.translatedLangText.TOTAL, ""];

  const pushTotal = (show: boolean, value: number) => {
    if (show) totalRow.push(value, "");
  };

  pushTotal(showGateSurcharge, totals.gateIn);
  pushTotal(showGateSurcharge, totals.gateOut);
  pushTotal(showSteamSurcharge, totals.steam);
  pushTotal(showCleanSurcharge, totals.clean);
  pushTotal(showRepairSurcharge, totals.repair);
  pushTotal(showResidueSurcharge, totals.residue);
  ...
  // AVERAGE ROW
  // =========================
  const avgRow: any[] = [this.translatedLangText.AVERAGE, ""];

  const pushAvg = (show: boolean, value: number) => {
    if (show) avgRow.push((value / monthCount).toFixed(2), "");
  };

  pushAvg(showGateSurcharge, totals.gateIn);
  pushAvg(showGateSurcharge, totals.gateOut);
  pushAvg(showSteamSurcharge, totals.steam);
  pushAvg(showCleanSurcharge, totals.clean);
  pushAvg(showRepairSurcharge, totals.repair);
  pushAvg(showResidueSurcharge, totals.residue);
  ...
  const addHeaderMerge = (show: boolean) => {
    if (show) {
  ...
  addHeaderMerge(showGateSurcharge);
  addHeaderMerge(showGateSurcharge);
  addHeaderMerge(showSteamSurcharge);
  addHeaderMerge(showCleanSurcharge);
  addHeaderMerge(showRepairSurcharge);
  addHeaderMerge(showResidueSurcharge);

  // =========================
  // TOTAL + AVERAGE MERGE
  // =========================
  const totalRowIndex = wsData.length - 2;
  const avgRowIndex = wsData.length - 1;

  // A:B merge (label)
  merges.push({ s: { r: totalRowIndex, c: 0 }, e: { r: totalRowIndex, c: 1 } });
  merges.push({ s: { r: avgRowIndex, c: 0 }, e: { r: avgRowIndex, c: 1 } });

  // value merges (2-column span)
  let c = 2;

  const addPairMerge = (show: boolean) => {
    if (show) {
      merges.push({ s: { r: totalRowIndex, c }, e: { r: totalRowIndex, c: c + 1 } });
      merges.push({ s: { r: avgRowIndex, c }, e: { r: avgRowIndex, c: c + 1 } });
      c += 2;
    }
  };

  addPairMerge(showGateSurcharge);
  addPairMerge(showGateSurcharge);
  addPairMerge(showSteamSurcharge);
  addPairMerge(showCleanSurcharge);
  addPairMerge(showRepairSurcharge);
  addPairMerge(showResidueSurcharge);
  ...
  for (let R = 0; R <= range.e.r; R++) {
    for (let C = 0; C <= range.e.c; C++) {
      ...
      const isTotalRow = R === wsData.length - 2;
      const isAvgRow = R === wsData.length - 1;
      const isFooter = isTotalRow || isAvgRow;

      cell.s = {
        font: {
          bold: isTitle || isHeader || isFooter,
          sz: isTitle ? 16 : isDate ? 12 : 10
        },
        alignment: {
          horizontal:
            (isTotalRow || isAvgRow) && C === 0
              ? "right"   // ✅ TOTAL / AVERAGE text right aligned
              : "center",
          vertical: "center"
        }
      };
  ...
  XLSX.writeFile(wb, this.GetReportTitle() + ".xlsx");
  ...
  async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    chartContentWidth = chartContentWidth * 0.95;
...

    {
      ...
      startY = PDFUtility.addReportSubTitle_Landscape(pdf, repGeneratedDate, pageWidth, leftMargin, rightMargin);
