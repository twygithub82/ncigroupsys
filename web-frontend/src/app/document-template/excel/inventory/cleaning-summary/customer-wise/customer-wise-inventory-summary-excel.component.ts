...
@Component({
  selector: 'app-customer-wise-inventory-summary-excel',
  templateUrl: './customer-wise-inventory-summary-excel.component.html',
  styleUrls: ['./customer-wise-inventory-summary-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class CustomerWiseInventorySummaryExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<CustomerWiseInventorySummaryExcelComponent>,
  ...
  const clnDate = `${this.translatedLangText.CLEANING_PERIOD}: ${this.date}`;
  ...
  for (let itm of this.report_summary_item) {
    data.push([
      (++idx).toString(),
      itm.code || "",
      itm.name || "",
  ...
  // Combine title + subtitle + header + data
  const wsData = [
    [reportTitle],         // Row 1: Title
    [clnDate],             // Row 2: Subtitle
    [],                    // Row 3: Empty
    headers,               // Row 4: Header
    ...data                // Data rows
  ];
  ...
  // Body styling
  const startRow = 4;
  const totalRowIndex = startRow + data.length - 1;

  for (let r = startRow; r <= totalRowIndex; r++) {
    for (let c = 0; c < headers.length; c++) {
      ...
      if (r === totalRowIndex) {
        ...
        ws[cellRef].s.fill = { fgColor: { rgb: "DDDDDD" } };

        // Merge TOTAL label across first 3 columns
        if (c === 0) {
          ws['!merges'].push({
            s: { r, c: 0 },
            e: { r, c: 2 }
  ...
  async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    for (let n = 0; n < this.report_summary_item.length; n++) {
      ...
      data.push([
        (++idx).toString(), itm.code || "", itm.name || "", itm.count || "0"
      ]);
    }

    data.push([this.translatedLangText.TOTAL, "", "", this.displayTotalValue()]);
    autoTable(pdf, {
      head: headers,
      body: data,
  ...
  GetWiseTitle(): string {
    return `${this.translatedLangText.CUSTOMER_WISE}`;
  }
