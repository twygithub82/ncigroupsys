...
@Component({
  selector: 'app-tariff-residue-cost-report-excel',
  templateUrl: './tariff-residue-cost-excel.component.html',
  styleUrls: ['./tariff-residue-cost-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class TariffResidueCostExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<TariffResidueCostExcelComponent>,
...
async exportExcel(items: TariffResidueItem[]) {
    const doc = new jsPDF();
    ...
    let fontSize = 12;
    doc.setFontSize(fontSize);
    doc.text("Residue Tariff", 105, 15, { align: "center" });
  ...
  this.translatedLangText.LAST_UPDATED
   ...
   const worksheet: XLSX.WorkSheet = XLSX.utils.aoa_to_sheet(rows);
    ...
    XLSX.writeFile(workbook, "ResidueDisposeTariff.xlsx");
    this.dialogRef.close();
}
