...
@Component({
  selector: 'app-steam-performance-detail-excel',
  templateUrl: './steam-detail-excel.component.html',
  styleUrls: ['./steam-detail-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class SteamPerformanceDetailExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  chartOptions: any;
  ...
  constructor(
    public dialogRef: MatDialogRef<SteamPerformanceDetailExcelComponent>,
...
exportToExcel_r1(fileName: string = 'report.xlsx') {
  ...
  const cutoffDate = PDFUtility.FormatColon(
    this.translatedLangText.STEAMING_PERIOD,
    this.date
  );
  ...
  // =========================
  // MULTI-ROW HEADER (like PDF)
  // =========================
  wsData.push([
    this.translatedLangText.S_N,
    this.translatedLangText.TANK_NO,
    this.translatedLangText.CODE,
    this.translatedLangText.CARGO,
    this.translatedLangText.COMPLETED_DATE,
    this.translatedLangText.DURATION,
    this.translatedLangText.REQUIRED_TEMP,
    this.translatedLangText.STEAM_COST,
    this.translatedLangText.BAY,
    this.translatedLangText.TEMPERATURE, "", "", "", "", ""
  ]);

  wsData.push([
    "", "", "", "", "", "", "", "", "",
    this.translatedLangText.THERMOMETER, "",
    this.translatedLangText.TOP_LASER, "",
    this.translatedLangText.BOTTOM_LASER, ""
  ]);

  wsData.push([
    "", "", "", "", "", "", "", "", "",
    this.translatedLangText.BEGIN,
    this.translatedLangText.CLOSE,
    this.translatedLangText.BEGIN,
    this.translatedLangText.CLOSE,
    this.translatedLangText.BEGIN,
    this.translatedLangText.CLOSE
  ...
  // DATA
  // =========================
  (this.repData || []).forEach((itm: any, i: number) => {
    wsData.push([
      i + 1,
      itm.tank_no || "",
      itm.customer_code || "",
      ...
      itm.cost || 0,
      itm.bay || "",
      itm.themometer?.begin_temp || "",
      itm.themometer?.close_temp || "",
      itm.top?.begin_temp || "",
      itm.top?.close_temp || "",
      itm.bottom?.begin_temp || "",
      itm.bottom?.close_temp || ""
    ]);
  });
