...
import { report_status_yard, report_status, AdminReportMonthlyReport, CustomerMonthlySales } from 'app/data-sources/reports';
...
export interface DialogData {
  repData: CustomerMonthlySales,
  date:string,
  repType:string,
  customer:string
}
@Component({
  selector: 'app-customer-sales-report-details-excel',
  templateUrl: './customer-sales-details-excel.component.html',
  styleUrls: ['./customer-sales-details-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class CustomerMonthlySalesReportDetailsExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<CustomerMonthlySalesReportDetailsExcelComponent>,
  ...
  // const headers = [
  //   this.GetReportColumnsHeader(), // First row header
  //   [
  ...
  const getExcelHeaderRow = () => {
  const headerRow: any[] = [
    this.translatedLangText.S_N,
    this.translatedLangText.CUSTOMER,
    this.translatedLangText.GATE_IN_QTY
  ];

  if (!this.modulePackageService.isStarterPackage()) {
    headerRow.push(this.translatedLangText.STEAM, '', this.translatedLangText.RESIDUE, '');
  }

  headerRow.push(
    this.translatedLangText.CLEANING, '',
    this.translatedLangText.IN_SERVICE, '',
    this.translatedLangText.OFFHIRE, ''
  );
...
const getExcelSubHeaderRow = () => {
  ...
  subHeaderRow.push(
    NoTankTitle, this.translatedLangText.COST, // CLEANING
    NoTankTitle, this.translatedLangText.COST, // IN_SERVICE
    NoTankTitle, this.translatedLangText.COST  // OFFHIRE
  );
...
const headers = [
  getExcelHeaderRow(),
  getExcelSubHeaderRow()
];

 

  // 3️⃣ Prepare body rows
  const data: any[][] = [];

  // Report title row
  data.push([reportTitle]);
  // Subtitle row
  data.push([repGeneratedDate]);
  // Blank row
  data.push([]);
  // Header rows
  data.push(...headers);
  ...
  for (let n = 0; n < (this.repData?.customer_sales?.length || 0); n++) {
    const itm = this.repData?.customer_sales?.[n];
    const row: any[] = [
      ++idx,
      itm?.name || '', // Surveyor/Customer (left-align)
      itm?.tank_in_count || ''
    ];
  ...
  // Total row
  const lastRow: any[] = [this.translatedLangText.TOTAL, '', this.repData?.total_tank_in];
  if (!this.modulePackageService.isStarterPackage()) {
    lastRow.push(
  ...
  ws['!cols'] = Utility.autoFitColumns(data);
  const lastCol = Math.max(...data.map(r => r.length)) - 1;

  // 5️⃣ Merge title and subtitle across all columns
  // ws['!merges'] = [
  //   { s: { r: 0, c: 0 }, e: { r: 0, c: lastCol } }, // Title
  //   { s: { r: 1, c: 0 }, e: { r: 1, c: lastCol } }, // Subtitle
  //   { s: { r: data.length - 1, c: 0 }, e: { r: data.length - 1, c: 1 } }, // TOTAL row merge

  //    // Row 0: merge multi-column headers (S_N, CUSTOMER, GATE_IN_QTY span 2 rows)
  //     { s: { r: 0, c: 0 }, e: { r: 1, c: 0 } }, // S_N
  //     { s: { r: 0, c: 1 }, e: { r: 1, c: 1 } }, // CUSTOMER
  //     { s: { r: 0, c: 2 }, e: { r: 1, c: 2 } }, // GATE_IN_QTY

  //     // Row 0: merge STEAM, RESIDUE, CLEANING, IN_SERVICE, OFFHIRE over 2 columns
  //     { s: { r: 0, c: 3 }, e: { r: 0, c: 4 } }, // STEAM
  //     { s: { r: 0, c: 5 }, e: { r: 0, c: 6 } }, // RESIDUE
  //     { s: { r: 0, c: 7 }, e: { r: 0, c: 8 } }, // CLEANING
  //     { s: { r: 0, c: 9 }, e: { r: 0, c: 10 } }, // IN_SERVICE
  //     { s: { r: 0, c: 11 }, e: { r: 0, c: 12 } }, // OFFHIRE
  // ];

  const headerMerges = [
  // Multi-row headers (S_N, CUSTOMER, GATE_IN_QTY)
  { s: { r: 3, c: 0 }, e: { r: 4, c: 0 } }, // S_N
  { s: { r: 3, c: 1 }, e: { r: 4, c: 1 } }, // CUSTOMER
  { s: { r: 3, c: 2 }, e: { r: 4, c: 2 } }, // GATE_IN_QTY

  // Multi-column headers
  { s: { r: 3, c: 3 }, e: { r: 3, c: 4 } }, // STEAM
  { s: { r: 3, c: 5 }, e: { r: 3, c: 6 } }, // RESIDUE
  { s: { r: 3, c: 7 }, e: { r: 3, c: 8 } }, // CLEANING
  { s: { r: 3, c: 9 }, e: { r: 3, c: 10 } }, // IN_SERVICE
  { s: { r: 3, c: 11 }, e: { r: 3, c: 12 } }, // OFFHIRE
];

// Title, subtitle, total row
const titleMerges = [
  { s: { r: 0, c: 0 }, e: { r: 0, c: lastCol } }, // Title
  { s: { r: 1, c: 0 }, e: { r: 1, c: lastCol } }, // Subtitle
  { s: { r: data.length - 1, c: 0 }, e: { r: data.length - 1, c: 1 } } // TOTAL
];

// Combine all
ws['!merges'] = [...titleMerges, ...headerMerges];
  ...
  for (let R = 0; R <= range.e.r; ++R) {
    for (let C = 0; C <= range.e.c; ++C) {
      ...
      // Subtitle row
        ...
        cell.s = { font: { sz: 12 }, alignment: { horizontal: 'right', vertical: 'center' } };
      }
      // Header rows
      else if (R === 3 || R === 4) {
        cell.s = {
      ...
      // Body rows
      else if (R > 4 && R < data.length - 1) {
        cell.s = {
          alignment: { horizontal: C === 1 ? 'left' : 'center', vertical: 'center' }, // Surveyor left-align
          // border: { top: { style: 'thin' }, bottom: { style: 'thin' }, left: { style: 'thin' }, right: { style: 'thin' } }
        };
      }
      // Total row
  ...
  // 7️⃣ Append worksheet and write file
 ...
 var fName=this.GetReportTitle()+'.xlsx';
  ...
  GetReportTitle(): string {
    var title:string='';
         title = `${this.translatedLangText.CUSTOMER_MONTHLY_SALES_REPORT}`
    return `${title}`
  }
