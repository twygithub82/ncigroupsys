...
import { report_status_yard, report_status, AdminReportMonthlyReport, DailyTeamRevenue } from 'app/data-sources/reports';
...
@Component({
  selector: 'app-daily-revenue-excel',
  templateUrl: './daily-revenue-excel.component.html',
  styleUrls: ['./daily-revenue-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class DailyRevenueExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<DailyRevenueExcelComponent>,
  ...
  exportToExcel_r1(fileName: string = 'report.xlsx') {
    ...
    const approvalDt = PDFUtility.FormatColon(
      this.translatedLangText.QC_DATE,
      this.date
    );
    ...
    // TOTAL row
    body.push([
      this.translatedLangText.TOTAL, "", "", "", "","","", totalRepairCost
    ]);

    // Combine all rows (Title + Subtitle + Table)
    const wsData = [
      [reportTitle],   // Row 1
      [approvalDt],    // Row 2
      [],              // Row 3 (gap)
    ...
    // Merge cells (Title, Approval, TOTAL)
    const totalRowIndex = body.length + 3; // offset due to title rows

    ws['!merges'] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: 7 } }, // Title
      { s: { r: 1, c: 0 }, e: { r: 1, c: 7 } }, // Approval Date
      { s: { r: totalRowIndex, c: 0 }, e: { r: totalRowIndex, c: 6 } } // TOTAL merge
    ];
    ...
    // Row heights (optional for better layout)
    ws['!rows'] = [
      { hpt: 24 }, // Title
      { hpt: 18 }, // Approval date
    ];

    // ✅ Styling (works if your xlsx build supports it)
    // Title center
    if (ws['A1']) {
      ws['A1'].s = {
    ...
    for (let col = 0; col < 7; col++) {
      ...
      if (ws[cellRef]) {
        ws[cellRef].s = {
          font: { bold: true },
          alignment: { horizontal: col === 5 ? 'left' : 'left' }
        };
      }
    }
  ...
  GetReportTitle(): string {
    var title: string = '';
    title = `${this.translatedLangText.DAILY_TEAM_REVENUE_REPORT}`
    return `${title}`
  }
