...
import { report_status_yard, report_status, DailyTeamApproval, DailyTeamRevenue } from 'app/data-sources/reports';
...
@Component({
  selector: 'app-daily-approval-excel',
  templateUrl: './daily-approval-excel.component.html',
  styleUrls: ['./daily-approval-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class DailyApprovalExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<DailyApprovalExcelComponent>,
  ...
  for (let n = 0; n < (this.repData?.length || 0); n++) {
    ...
    body.push([
      ...
      repairCost,
      this.displayProcessStatus(itm?.status!)
    ]);
  }
  ...
  ws['!merges'] = [
    ...
    { s: { r: 1, c: 0 }, e: { r: 1, c: 6 } }, // Approval Date
    { s: { r: totalRowIndex, c: 0 }, e: { r: totalRowIndex, c: 4 } } // TOTAL merge
  ];
