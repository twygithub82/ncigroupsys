...
export interface DialogData {
  repData: AdminReportMonthlySalesReport,
  date: string,
  repType: string,
  customer: string,
...
@Component({
  selector: 'app-monthly-report-details-excel',
...
export class MonthlySalesReportDetailsExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  repData?: AdminReportMonthlySalesReport;//ManagementReportMonthlyRevenueItem;
  date?: string;
  repType?: string;
  customer?: string;
  index: number = 0;
  ...
  constructor(
    public dialogRef: MatDialogRef<MonthlySalesReportDetailsExcelComponent>,
 ...
 exportToExcel_r1(fileName: string = 'report.xlsx') {
  ...
  for (const date in grpData) {
    ...
    if (showGateSurcharge) pushData(entry.gate);
  ...
  if (showGateSurcharge) pushTotal(this.repData?.gate_monthly_sales);
  if (showPreinspectSurcharge) pushTotal(this.repData?.preinspection_monthly_sales);
  ...
  if (showGateSurcharge) pushAvg(this.repData?.gate_monthly_sales);
  if (showPreinspectSurcharge) pushAvg(this.repData?.preinspection_monthly_sales);
...
fileName=this.GetReportTitle()+'.xlsx';
  ...
  async exportToPDF_r1(fileName: string = 'document.pdf') {
    ...
    var grpData = InventoryAnalyzer.groupSalesMonthlyByDate(repData!);
  ...
  getValidItem(): any {
    var repData: any = this.data.repData;
    if (repData?.cleaning_monthly_sales) return repData.cleaning_monthly_sales;
    if (repData?.lolo_monthly_sales) return repData.lolo_monthly_sales;
    if (repData?.preinspection_monthly_sales) return repData.preinspection_monthly_sales;
    if (repData?.repair_monthly_sales) return repData.repair_monthly_sales;
    if (repData?.residue_monthly_sales) return repData.residue_monthly_sales;
    if (repData?.steaming_monthly_sales) return repData.steaming_monthly_sales;
