...
import { report_status_yard, report_status, AdminReportYearlyReport, InventoryAnalyzer, AdminReportYearlySalesReport } from 'app/data-sources/reports';
...
@Component({
  selector: 'app-yearly-report-details-excel',
  templateUrl: './yearly-details-excel.component.html',
  styleUrls: ['./yearly-details-excel.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    MatButtonModule,
...
export class YearlySalesReportDetailsExcelComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
 ...
 lineChartData: ChartConfiguration['data'] = {
  ...
  constructor(
    public dialogRef: MatDialogRef<YearlySalesReportDetailsExcelComponent>,
...
async exportToExcel_r1(fileName: string = 'document.xlsx') {
      ...
      const grpData =InventoryAnalyzer.groupByMonthAndFindExtremes_Sales(this.repData!);
      ...
      const pushHeader = (title: string) => {
        ...
        headerRow2.push(NoTankTitle, this.translatedLangText.COST);
      };
    
      if (showGateSurcharge) pushHeader(this.translatedLangText.GATEIO);
      if (showPreinspectSurcharge) pushHeader(this.translatedLangText.PREINSPECTION);
      ...
      for (const date in grpData.monthlyData) {
        const entry = grpData.monthlyData[date];
        ...
        const pushData = (obj: any) => {
          row.push(obj?.count || "");
          row.push(obj?.cost || "0.00");
        };
    
        if (showGateSurcharge) pushData(entry.in_out);
        if (showPreinspectSurcharge) pushData(entry.preinspection);
      ...
      // TOTAL ROW
      // =========================
      let totalRow: any[] = [this.translatedLangText.TOTAL, ""];
    
      const pushTotal = (obj: any) => {
        totalRow.push(obj?.total_count || "");
        totalRow.push(obj?.total_cost || "");
      };
    
      if (showGateSurcharge) pushTotal(this.repData?.gate_yearly_sales);
      if (showPreinspectSurcharge) pushTotal(this.repData?.preinspection_yearly_sales);
      if (showLoloSurcharge) pushTotal(this.repData?.lolo_yearly_sales);
      if (showStorageSurcharge) pushTotal(this.repData?.storage_yearly_sales);
      if (showSteamSurcharge) pushTotal(this.repData?.steaming_yearly_sales);
      if (showCleanSurcharge) pushTotal(this.repData?.cleaning_yearly_sales);
      if (showResidueSurcharge) pushTotal(this.repData?.residue_yearly_sales);
      if (showRepairSurcharge) pushTotal(this.repData?.repair_yearly_sales);
      ...
      // AVERAGE ROW
      // =========================
      let avgRow: any[] = [this.translatedLangText.AVERAGE, ""];
    
      const pushAvg = (obj: any) => {
        avgRow.push(obj?.average_count || "");
        avgRow.push(obj?.average_cost || "");
      };
    
      if (showGateSurcharge) pushAvg(this.repData?.gate_yearly_sales);
      if (showPreinspectSurcharge) pushAvg(this.repData?.preinspection_yearly_sales);
      if (showLoloSurcharge) pushAvg(this.repData?.lolo_yearly_sales);
      if (showStorageSurcharge) pushAvg(this.repData?.storage_yearly_sales);
      if (showSteamSurcharge) pushAvg(this.repData?.steaming_yearly_sales);
      if (showCleanSurcharge) pushAvg(this.repData?.cleaning_yearly_sales);
      if (showResidueSurcharge) pushAvg(this.repData?.residue_yearly_sales);
      if (showRepairSurcharge) pushAvg(this.repData?.repair_yearly_sales);
      ...
      if (showGateSurcharge) addMerge();
      if (showPreinspectSurcharge) addMerge();
      ...
      for (let R = 0; R <= range.e.r; R++) {
        for (let C = 0; C <= range.e.c; C++) {
          ...
          if (R === 3 || R === 4) {
            ...
            // style.fill = { fgColor: { rgb: "D3D3D3" } };
          }
    
          // TOTAL & AVERAGE
          if (R >= range.e.r - 1) {
            style.font = { bold: true };
            // style.fill = { fgColor: { rgb: "E7E7E7" } };
          }
      ...
      // AUTO WIDTH
      // =========================
      ws['!cols'] = sheetData[3].map(() => ({ wch: 15 }));
...
//     // Utility.AddTextAtCenterPage(pdf, repGeneratedDate, pageWidth, leftMargin, rightMargin + 5, startY - 5, PDFUtility.CenterSubTitleFontSize());
//     var subtitlePos =1;
...
//     var repdata=this.repData!;
//     var grpData= InventoryAnalyzer.groupByMonthAndFindExtremes_Sales(this.repData!);
...
//       margin: { top: startY,left: leftMargin }, // top margin for all pages
//       theme: 'grid',
...
//           if (((data.column.index % 2 == 0 && data.column.index != 0) || data.column.index == 0)) {
...
//  async exportToPDF_r11(fileName: string = 'document.pdf') {
//     const pageWidth = 297; // A4 width in mm (landscape)
//     const pageHeight = 220; // A4 height in mm (landscape)
//     const leftMargin = 10;
//     const rightMargin = 10;
...
//     var grpData= InventoryAnalyzer.groupByMonthAndFindExtremes_Sales(repData!);
...
//       data.push([
//       this.translatedLangText.TOTAL,"",
//       ...(showGateSurcharge?[total_tank_In_out,Utility.formatNumberDisplay(repData?.gate_yearly_revenue?.total_cost)]:[]),
//       ...(showPreinspectSurcharge?[total_tank_preinspect,Utility.formatNumberDisplay(repData?.preinspection_yearly_revenue?.total_cost)]:[]),
//       ...(showLoloSurcharge?[total_tank_lolo,Utility.formatNumberDisplay(repData?.lolo_yearly_revenue?.total_cost)]:[]),
//       ...(showStorageSurcharge?[total_tank_storage,Utility.formatNumberDisplay(repData?.storage_yearly_revenue?.total_cost)]:[]),
//       ...(showSteamSurcharge?[total_tank_steam,Utility.formatNumberDisplay(repData?.steam_yearly_revenue?.total_cost)]:[]),
//       ...(showResidueSurcharge?[total_tank_residue,Utility.formatNumberDisplay(repData?.residue_yearly_revenue?.total_cost)]:[]),
//       ...(showCleanSurcharge?[total_tank_clean,Utility.formatNumberDisplay(repData?.cleaning_yearly_revenue?.total_cost)]:[]),
//       ...(showRepairSurcharge?[total_tank_repair,Utility.formatNumberDisplay(repData?.repair_yearly_revenue?.total_cost)]:[]),
//       Utility.formatNumberDisplay(total_all_cost)
//     ]);

//     data.push([
//       this.translatedLangText.AVERAGE,"",
//       ...(showGateSurcharge?[average_tank_In_out,Utility.formatNumberDisplay(repData?.gate_yearly_revenue?.average_cost||'')]:[]),
//       ...(showPreinspectSurcharge?[average_tank_preinspect,Utility.formatNumberDisplay(repData?.preinspection_yearly_revenue?.average_cost||'')]:[]),
//       ...(showLoloSurcharge?[average_tank_lolo,Utility.formatNumberDisplay(repData?.lolo_yearly_revenue?.average_cost||'')]:[]),
//       ...(showStorageSurcharge?[average_tank_storage,Utility.formatNumberDisplay(repData?.storage_yearly_revenue?.average_cost||'')]:[]),
//       ...(showSteamSurcharge?[average_tank_steam,Utility.formatNumberDisplay(repData?.steam_yearly_revenue?.average_cost||'')]:[]),
//       ...(showResidueSurcharge?[average_tank_residue,Utility.formatNumberDisplay(repData?.residue_yearly_revenue?.average_cost||'')]:[]),
//       ...(showCleanSurcharge?[average_tank_clean,Utility.formatNumberDisplay(repData?.cleaning_yearly_revenue?.average_cost||'')]:[]),
//       ...(showRepairSurcharge?[average_tank_repair,Utility.formatNumberDisplay(repData?.repair_yearly_revenue?.average_cost||'')]:[]),  
//       Utility.formatNumberDisplay((total_all_cost/average_counter))
//     ]);
...
//     prcsValues=[
//       ...(showGateSurcharge?[(repData?.gate_yearly_revenue?.total_cost||0)]:[]),
//       ...(showPreinspectSurcharge?[(repData?.preinspection_yearly_revenue?.total_cost||0)]:[]),
//       ...(showLoloSurcharge?[(repData?.lolo_yearly_revenue?.total_cost||0)]:[]),
//       ...(showStorageSurcharge?[(repData?.storage_yearly_revenue?.total_cost||0)]:[]),
//       ...(showSteamSurcharge?[(repData?.steam_yearly_revenue?.total_cost||0)]:[]),
//       ...(showResidueSurcharge?[(repData?.residue_yearly_revenue?.total_cost||0)]:[]),
//       ...(showCleanSurcharge?[(repData?.cleaning_yearly_revenue?.total_cost||0)]:[]),
//       ...(showRepairSurcharge?[(repData?.repair_yearly_revenue?.total_cost||0)]:[]) ];
