...
export class JobOrderCleaningComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  onFilterCleaning() {
    ...
    if (this.filterCleanForm!.get('filterClean')?.value) {
      where.or = [
        ...
        { estimate_no: { contains: this.filterCleanForm!.get('filterClean')?.value } }
      ];
    }

    this.lastSearchCriteriaRepair = this.cleanDS.addDeleteDtCriteria(where);
    this.performSearchClean(this.pageSizeRepair, this.pageIndexRepair, this.pageSizeRepair, undefined, undefined, undefined, () => { });
  }
  ...
  performSearchClean(pageSize: number, pageIndex: number, first?: number, after?: string, last?: number, before?: string, callback?: () => void) {
    this.subs.sink = this.cleanDS.search(this.lastSearchCriteriaRepair, this.lastOrderByRepair, first, after, last, before)
      .subscribe(data => {
        this.clnEstList = data;
        this.endCursorRepair = this.cleanDS.pageInfo?.endCursor;
        this.startCursorRepair = this.cleanDS.pageInfo?.startCursor;
        this.hasNextPageRepair = this.cleanDS.pageInfo?.hasNextPage ?? false;
        this.hasPreviousPageRepair = this.cleanDS.pageInfo?.hasPreviousPage ?? false;
      });
