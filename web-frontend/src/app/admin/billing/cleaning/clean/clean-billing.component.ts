...
import { pageSizeInfo, TANK_STATUS_POST_IN_YARD, Utility, BILLING_TANK_STATUS, BILLING_TANK_STATUS_IN_YARD, BILLING_ESTIMATE_STATUS } from 'app/utilities/utility';
...
export class CleanBillingComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  save(event: Event) {
    ...
    this.billDS.searchCleaningBilling(where).subscribe(b => {
      ...
      if (b.length) {
        var fiteredb = b.filter(b => (b.cleaning?.length || 0) > 0);
  ...
  SaveNewBilling(event: Event) {
    ...
    newBilling.invoice_type = invoice_type_mapping.CLEANING;
