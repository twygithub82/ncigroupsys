...
@Component({
  selector: 'app-client-approval-view',
  standalone: true,
  templateUrl: './client-approval-view.component.html',
  styleUrl: './client-approval-view.component.scss',
  imports: [
    BreadcrumbComponent,
    MatButtonModule,
    MatSidenavModule,
...
export class RepairApprovalClientViewComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  displayedColumns = [
    'seq',
    'subgroup_name_cv',
    'damange',
  ...
  breadcrumsMiddleList = [
    { text: 'MENUITEMS.REPAIR.TEXT', route: '/admin/repair/client-approval' },
    { text: 'MENUITEMS.REPAIR.LIST.CLIENT-APPROVAL', route: '/admin/repair/client-approval' }
  ]
  ...
  langText = {
    ...
    APPROVE_SUCCESS: 'COMMON-FORM.ACTION-SUCCESS',
    ...
    if (this.repair_guid) {
      this.subs.sink = this.repairDS.getRepairByIDForApproval(this.repair_guid).subscribe(data => {
        if (data?.length) {
          ...
          this.getCustomerMaxDiscount(this.sotItem?.storing_order?.customer_company?.guid!);
  ...
  populateRepair(repair: RepairItem) {
    ...
    this.previousOverallCost = repair.total_cost || 0;
  ...
  onApprove(event: Event) {
    ...
    if (this.repairForm?.valid) {
      ...
      re.total_cost = Utility.convertNumber(this.repairForm?.get('net_cost')?.value, 2); // approved total
      re.total_hour = Utility.convertNumber(this.repairForm?.get('total_hour')?.value, 2); // approved total
      re.est_cost = Utility.convertNumber(this.repairForm?.get('net_cost_est')?.value, 2); // estimate total
      re.total_labour_cost = Utility.convertNumber(this.repairForm?.get('total_labour_cost')?.value, 2); // approved total
      re.total_material_cost = Utility.convertNumber(this.repairForm?.get('total_mat_cost')?.value, 2); // approved total
  ...
  handleSaveSuccess(count: any) {
    if ((count ?? 0) > 0) {
      ...
      this.router.navigate(['/admin/repair/client-approval']);
    }
  }
  ...
  isAllowEdit() {
    return this.modulePackageService.hasFunctions(['REPAIR_ESTIMATE_APPROVAL_CLIENT']);
  }
  ...
  hasCostDecreasedByQuarter(): boolean | null {
    const currentCost = Utility.convertNumber(this.repairForm?.get('net_cost')?.value, 2); // approved total
