...
export class RepairEstimateComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructSearchCriteria(): boolean {
    ...
    const where: any = {
      tank_status_cv: { in: ['REPAIR', 'STORAGE'] }, // TODO :: Test fo Malaysia Depot allow to add repair after in gate survey, FOR: ['CLEANING', 'REPAIR', 'STORAGE']
      purpose_repair_cv: { in: ["REPAIR", "OFFHIRE"] } // TODO :: Test fo Malaysia Depot allow to add repair after in gate survey, FOR: ["", "REPAIR", "OFFHIRE"]
    };
  ...
  anyActiveResidues(sot: StoringOrderTankItem): boolean {
    return BusinessLogicUtil.anyActiveResidues(sot.residue);  // TODO :: Test fo Malaysia Depot allow to add repair after in gate survey, FOR: false;
  }
