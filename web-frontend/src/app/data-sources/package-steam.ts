...
export class PackageSteamingDS extends BaseDataSource<PackageSteamingItem> {
  ...
  SearchAllPackageSteam(
  where?: any,
  order?: any
): Observable<PackageSteamingItem[]> {
  this.loadingSubject.next(true);

  const pageSize = 100; // safe page size
  let allData: PackageSteamingItem[] = [];

  const fetchPage = (after?: string): Observable<PackageSteamingItem[]> => {
    return this.apollo
      .query<any>({
        query: GET_PACKAGE_STEAM_QUERY,
        variables: {
      ...
      .pipe(
        map(res => res.data.packageSteamResult),
        switchMap(result => {
