...
export class InGateSurveyGO {
  ...
  public airline_valve_oth?: string;
  public airline_valve_conn_cv?: string;
  public airline_valve_conn_oth?: string;
  public airline_valve_conn_spec_cv?: string;
  public airline_valve_conn_spec_oth?: string;
  public manlid_cover_cv?: string;
  public manlid_cover_oth?: string;
...
export const QUERY_IN_GATE_SURVEY_BY_ID_FOR_EIR_PDF = gql`
  query queryInGateSurveyByID($where: in_gate_surveyFilterInput){
    resultList: queryInGateSurvey(where: $where) {
      totalCount
      nodes {
        airline_valve_conn_cv
        airline_valve_conn_spec_cv
...
export class InGateSurveyDS extends BaseDataSource<InGateSurveyItem> {
  ...
  getInGateSurveyByIDForEirPdf(guid: string): Observable<InGateSurveyItem[]> {
    this.loadingSubject.next(true);
    let where: any = { guid: { eq: guid }, delete_dt: { eq: null } }
    return this.apollo
      .query<any>({
        query: QUERY_IN_GATE_SURVEY_BY_ID_FOR_EIR_PDF,
        variables: { where },
        fetchPolicy: 'no-cache' // Ensure fresh data
      })
      .pipe(
