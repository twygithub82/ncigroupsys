...
export class TariffSteamingDS extends BaseDataSource<TariffSteamingItem> {
  ...
  SearchAllTariffSteam(where?: any, order?: any): Observable<TariffSteamingItem[]> {
  this.loadingSubject.next(true);

  const pageSize = 100; // use max allowed by backend
  let allData: TariffSteamingItem[] = [];

  const fetchPage = (after?: string): Observable<TariffSteamingItem[]> => {
    return this.apollo
      .query<any>({
        query: GET_TARIFF_STEAM_QUERY,
        variables: {
      ...
      .pipe(
        map(res => res.data.tariffSteamResult),
        switchMap(result => {
