...
export const QUERY_OUT_GATE_SURVEY_BY_ID_FOR_EIR_PDF = gql`
  query queryOutGateSurveyByID($where: out_gate_surveyFilterInput){
    resultList: queryOutGateSurvey(where: $where) {
      totalCount
      nodes {
...
export class OutGateSurveyDS extends BaseDataSource<OutGateSurveyItem> {
  ...
  getOutGateSurveyByIDForEirPdf(guid: string): Observable<OutGateSurveyItem[]> {
    this.loadingSubject.next(true);
    let where: any = { guid: { eq: guid }, delete_dt: { eq: null } }
    return this.apollo
      .query<any>({
        query: QUERY_OUT_GATE_SURVEY_BY_ID_FOR_EIR_PDF,
        variables: { where },
        fetchPolicy: 'no-cache' // Ensure fresh data
      })
      .pipe(
