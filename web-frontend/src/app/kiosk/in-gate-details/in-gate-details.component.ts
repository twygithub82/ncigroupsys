...
@Component({
  selector: 'app-kiosk-in-gate-details',
  standalone: true,
...
export class InGateDetailsComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  populateInGateForm(sot: StoringOrderTankItem): void {
    this.inGateForm!.patchValue({
      ...
      yard_cv: this.igDS.getInGateItem(sot.in_gate)?.yard_cv || this.yardId,
  ...
  goToPrevious() {
    this.router.navigate([`/kiosk/${this.yardId?.toLowerCase()}/in-gate`]);
  }
